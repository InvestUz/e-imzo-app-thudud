...
<div class="block">
    <div class="section-heading">
        ...
        <a href="{{ route('admin.roles') }}" class="platon-btn platon-btn-outline platon-btn-sm" style="margin-left:auto;font-size:0.78rem">Rol & Huquqlar ›</a>
    </div>
    <div class="platon-table-wrap">
        <table class="platon-table">
            ...
            @php $activeSession = $user->activeSessions->first(); @endphp
            <tr>
                <td>
                ...
                <td>
                    ...
                    <span class="sbadge {{ $user->isAdmin() ? 'sbadge-purple' : ($user->isConsumer() ? 'sbadge-gray' : 'sbadge-blue') }}">{{ $rl }}</span>
                    @if($user->commission_position)
                    <div style="font-size:0.72rem;color:#6e788b;margin-top:2px">{{ $user->commission_position }}</div>
                    @endif
                </td>
                <td>{{ $user->district?->name_uz ?? '—' }}</td>
                <td>
                    @if($user->is_regional_backup)
                    <span class="sbadge sbadge-success" title="Barcha tumanlar uchun tasdiqlashi mumkin" style="font-size:0.72rem">✓ Ha</span>
                    @else
                    <span class="sbadge sbadge-gray" style="font-size:0.72rem">Yo'q</span>
                    @endif
                    <form action="{{ route('admin.users.toggle-backup', $user) }}" method="POST" style="display:inline;margin-left:4px">
                        @csrf
                        <button type="submit" class="platon-btn platon-btn-outline platon-btn-sm" style="padding:2px 7px;font-size:0.68rem" title="O'zgartirish">
                            {{ $user->is_regional_backup ? "O'ch" : 'Yoq' }}
                        </button>
                    </form>
                </td>
                <td>
                ...
                <td>
                    ...
                    <div style="font-size:0.7rem;color:#6e788b;margin-top:2px">{{ $activeSession->ip_address }}</div>
                    @else
                    <span class="sbadge sbadge-gray" style="font-size:0.72rem">Offline</span>
                    @endif
                </td>
                <td>
                    <div style="display:flex;gap:5px;flex-wrap:wrap">
                        ...
                        @if($activeSession)
                        <form action="{{ route('admin.users.force-logout', $user) }}" method="POST" onsubmit="return confirm('{{ $user->name }} ni tizimdan chiqarasizmi?')">
                            @csrf
                            <button type="submit" class="platon-btn platon-btn-danger platon-btn-sm">Chiqarish</button>
                        </form>
                        @endif
                        <button onclick="openNotify({{ $user->id }}, '{{ addslashes($user->name) }}')" class="platon-btn platon-btn-outline platon-btn-sm">Xabar</button>
                        @if(auth()->id() !== $user->id)
            ...
            @empty
            <tr><td colspan="7" style="text-align:center;padding:40px;color:#aab0bb">Foydalanuvchilar topilmadi</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="district_id" class="form-select">
                        ...
                        <option value="{{ $d->id }}">{{ $d->name_uz }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="form-label small fw-semibold">Lavozim (komissiya uchun)</label>
                    <input type="text" name="commission_position" class="form-control" maxlength="100" placeholder="Masalan: Kadastr mutaxassisi">
                </div>
                <div class="col-12">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="is_regional_backup" value="1" id="cb-backup-create">
                        <label class="form-check-label small" for="cb-backup-create">
                            <strong>Mintaqaviy zaxira</strong> — barcha tumanlar arizalarini ko'ra/tasdiqlaya oladi
                        </label>
                    </div>
                </div>
                <div class="col-12">
...
<div id="notify-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:500;align-items:center;justify-content:center">
    <div style="background:#fff;border-radius:20px;padding:28px;width:460px;max-width:95vw;position:relative">
        <button onclick="document.getElementById('notify-modal').style.display='none'" style="position:absolute;top:16px;right:16px;background:none;border:none;font-size:1.2rem;cursor:pointer;color:#6e788b">✕</button>
        <h5 style="font-size:0.95rem;font-weight:700;margin-bottom:16px">Xabar yuborish: <span id="notify-username"></span></h5>
        <form id="notify-form" method="POST">
            @csrf
            <div style="margin-bottom:12px">
                <label style="font-size:0.82rem;color:#6e788b;display:block;margin-bottom:4px">Sarlavha</label>
                <input type="text" name="title" class="form-control" maxlength="255" required placeholder="Xabar sarlavhasi">
            </div>
            <div style="margin-bottom:16px">
                <label style="font-size:0.82rem;color:#6e788b;display:block;margin-bottom:4px">Matn</label>
                <textarea name="body" class="form-control" rows="3" maxlength="1000" placeholder="Qo'shimcha ma'lumot (ixtiyoriy)"></textarea>
            </div>
            <button type="submit" class="platon-btn platon-btn-primary" style="width:100%">Yuborish</button>
        </form>
    </div>
</div>
...
function openNotify(userId, userName) {
    document.getElementById('notify-username').textContent = userName;
    document.getElementById('notify-form').action = '/admin/users/' + userId + '/notify';
    document.getElementById('notify-modal').style.display = 'flex';
}
['create-modal','notify-modal'].forEach(function(id){
